...
class ADMIN_HELPER extends BASE_HELPER
{
    ...
    static function createAdmin($request)
    {
        ...
        #=====ENVOIE D'SMS =======~####
        $message = "Votre compte admin a été crée avec succès sur BLS. Voici ci-dessous vos identifiants de connexion: Username::" . $username . "; Password:: " . $username;
        ...
        try {
           ...
           // );
            ...
            Send_Notification(
                $user,
                "CREATION DE COMPTE ADMIN SUR BLS",
                $message
            );
        } catch (\Throwable $th) {
            //throw $th;
    ...
    {
        $admins =  Admin::with(["as_user", 'parent', 'owner'])->where(["owner" => request()->user()->id])->orderBy("id", "desc")->get();
        return self::sendResponse($admins, 'Tout les admins récupérés avec succès!!');
    }

    static function retrieveAdmins($id)
    {
        $admin = Admin::with(["as_user", 'parent', 'owner'])->where(["owner" => request()->user()->id, "id" => $id])->get();
        if ($admin->count() == 0) {
