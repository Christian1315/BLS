...
class TRANSACTION_HELPER extends BASE_HELPER
{
    ...
    static function createTransaction($request)
    {
        ...
        ### VERIFIONS SI LA Clé API EST VALIDE(voyons si elle appartient au user en question)
        ...
        $client = Client::where(["email" => $clientData['email'], "phone" => $clientData['phone']])->first();
    ...
    {
        $transaction = Transaction::with(["_type", "_client", "_status","_module"])->find($id);
        if (!$transaction) {
    ...
    {
        $transactions = Transaction::with(["_type", "_client", "_status","_module"])->orderBy('id', 'desc')->get();
